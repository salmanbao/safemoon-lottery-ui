...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInSafemoon } = lotteryNodeData
    prizeInBusd = amountCollectedInSafemoon.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" SAFEMOON"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInSafemoon)}
            decimals={0}
          />
        )}
      </>
